...
{
	GPIO_EnumErrorState GPIO_LocalErrorState = GPIO_NOK;
	if ( (Pin >= GPIO_PIN0) && (Pin <= GPIO_PIN16) && (ReadedValue != NULL) )
	{
		GPIO_LocalErrorState = GPIO_OK;
		/*This line returns the bit value at its location but we want to return 1 or 0 only*/
		*ReadedValue = ((GPIO_Peripheral_t*)Port)->GPIOx_IDR & (1<<Pin);
...
{
	GPIO_EnumErrorState GPIO_LocalErrorState = GPIO_NOK;
	if ( (Pin >= GPIO_PIN0) && (Pin <= GPIO_PIN16) )
	{
		GPIO_LocalErrorState = GPIO_OK;
		switch (OutValue)
		{
		case GPIO_HIGH:
			((GPIO_Peripheral_t*)Port)->GPIOx_ODR = ((GPIO_Peripheral_t*)Port)->GPIOx_ODR | (1<<Pin);
			break;
		case GPIO_LOW:
			((GPIO_Peripheral_t*)Port)->GPIOx_ODR = ((GPIO_Peripheral_t*)Port)->GPIOx_ODR & (~(1<<Pin));
			break;
...
{
	GPIO_EnumErrorState GPIO_LocalErrorState = GPIO_NOK;
		if ( (Pin >= GPIO_PIN0) && (Pin < GPIO_PIN8) )
		{
			GPIO_LocalErrorState = GPIO_OK;
			uint32 Temp;
			Temp = (((GPIO_Peripheral_t*)Port)->GPIOx_AFRL);
			Temp &= ~ (0x0f<<(Pin*4));
			Temp |= (AF<<(Pin*4));
			(((GPIO_Peripheral_t*)Port)->GPIOx_AFRL) = Temp;
		...
		else if ((Pin >= GPIO_PIN8) && (Pin < GPIO_PIN15))
		{
			GPIO_LocalErrorState = GPIO_OK;
			uint32 Temp;
			Temp = (((GPIO_Peripheral_t*)Port)->GPIOx_AFRH);
			Temp &= ~ (0x0f<<((Pin-8)*4));
			Temp |= (AF<<((Pin-8)*4));
			(((GPIO_Peripheral_t*)Port)->GPIOx_AFRH) = Temp;
